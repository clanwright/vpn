...
the domain's exact application-package input. AdGuard Home and Unbound use the
domain's pinned platform context. NaiveProxy is the domain-owned manual binary
package because the required package is unavailable in the selected official
package stream.
