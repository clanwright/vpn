...
Python запрещён, включая тестовые harness: readiness и DNS-проверки определены
в Nix и используют пакетные CLI. Сборка первоначального избыточного override
была остановлена. Тяжёлые проверки и использование существующего builder
приостановлены; новый runtime suite и полный gate не выполнены. Лёгкие
статические проверки и source review учитываются отдельно.

Независимое source review завершено без существенных открытых замечаний.
Исправлены обходы DNSSEC/include, отделены семейства исходящей рекурсии от
listener-адресов; отрицательные проверки изолированы друг от друга. Проверка
общей Clan-композиции прошла на уровне evaluation. Репозиторная runtime-приёмка
остаётся незакрытой до разрешённого запуска проверок без VM-backed builder.
...
VM в проекте запрещены. Приёмка кандидата использует Nix evaluation, parser
и обычные Linux-процессы; реальная активация systemd и Internet/клиентская
приёмка выполняются позже. Сначала завершаются все VPN-аудиты, затем релиз
...
| Проверка | Есть настоящий тест `READY=1`, но он использует минимальный тестовый конфиг |
