...
и реализована в [Apple-клиенте версии 1.14.0](https://github.com/SagerNet/sing-box-for-apple/blob/59540eb0e1812bb76a481a9dc3dec6a788f4196f/Library/Network/ExtensionPlatformInterface.swift#L43-L170).
...
Точное дополнение содержит 47 IPv4 и 134 IPv6 маршрута. Исходники подтверждают
преобразование списка в настройки Apple Network Extension, но не совместимость
с одновременной работой SFM и Tailscale на macOS.
...
диапазонов, но не выбор маршрута операционной системой. Для vpn 0.9.1,
SFM 1.14.0 и macOS 26.5.2 оператор сообщил: после обнаружения `en0` и применения
TUN появляется `missing default interface`, затем 137 ошибок
`no available network interface` примерно за семь секунд; Tailscale подключён.
Причина потери доступного пути не установлена. Положительный список маршрутов
остаётся гипотезой, а не доказанным источником сбоя.

Переход на default routes без исключений — кандидат для отдельного сравнения,
не подтверждённое исправление. Если локальный трафик попадёт в TUN, правило
`DIRECT` само по себе не гарантирует выход через Tailscale: автоопределение может
привязать исходящий сокет к физическому интерфейсу. Отключение
`auto_detect_interface` не отключает монитор доступности сети SFM.
